...
public class MoveToClosestTargetEditor : Editor
{
    public void OnSceneGUI()
    {
        ...
        for (int i = 0; i < moveToClosestTarget.ResourceNodes.Length; i++)
        {
            Paths.Add(new NavMeshPath());
            if (moveToClosestTarget.ResourceNodes[i] == null)
            {
                continue;
            }
            NavMeshPath Path = Paths[i];

            if (NavMesh.CalculatePath(moveToClosestTarget.transform.position, moveToClosestTarget.ResourceNodes[i].position, moveToClosestTarget.Agent.areaMask, Path))
            {
                ...
                Handles.Label(moveToClosestTarget.ResourceNodes[i].position, $"Vector3 Distance: {Vector3.Distance(moveToClosestTarget.transform.position, moveToClosestTarget.ResourceNodes[i].position).ToString("N3")}\r\nPath Distance: {distance.ToString("N3")}", Style);
            }
            else
            {
